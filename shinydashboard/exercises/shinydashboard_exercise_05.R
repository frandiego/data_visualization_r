...
############################## SELECT INPUT ALPHA
select_input_alpha <-    selectInput(inputId = 'id_ggplot_alpha',
                                    choices = names(mpg)  , 
                                    selected = 'class',
                                    label = 'Alpha Aesthetic',
                                    multiple = F,
                                    selectize = T)

############################## SELECT INPUT ALPHA
select_input_size_range <-    sliderInput(inputId = 'id_size_factor',
                                         label = 'Size Factor',
                                         min = 1,
                                         max = 30,
                                         value = 5,
...
############################## CHECKBOX JITTER
checkbox_jitter <-    checkboxInput(inputId = 'id_jitter',
                                            label = 'Jitter',
                                            value = F)
  ...
  observeEvent(eventExpr = input$id_name,
               handlerExpr = {
                 ...
                 updateSliderInput(session = session,
                                   ...
                                   label = ticker,
                                   value = ticker)})
  # OBSERVER TICKER -> NAME
  observeEvent(eventExpr = input$id_ticker,
               handlerExpr = {
                 name <- symbols[Symbol == input$id_ticker,Name]
                 updateSliderInput(session = session,
                                   inputId = 'id_name',
                                   label = name,
                                   value = name)})
...
############################################################ GGPLOT ------------
  output$plotly_scatter <- renderPlotly({
    ...
    aes_alpha   <- paste0('factor(',input$id_ggplot_alpha,')')
...
############################################################ MAP ---------------
  output$map <- renderLeaflet({
    gsp <- ggmap::geocode(location = input$id_map_text,source = 'dsk')
    leaflet() %>% addTiles() %>% addMarkers(lat = as.numeric(gsp$lat),
                                            lng = as.numeric(gsp$lon),
                                            popup = input$id_map_text) 
  })
